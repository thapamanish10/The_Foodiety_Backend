...
@section('content')
    <main class="dashboardContainer">
        ...
        <div class="detailsformBody">
            <form action="{{ route("blog.update", [$data->id]) }}" id="" class="form" method="POST"
                ...
                <div class="formBody">
                    <div class="formGroup">
                        ...
                        <input type="text" name="blog_title" id="blog_title" value="{{ $data->blog_title }}">
                    </div>
                    <div class="row">
                        <div class="formGroup">
                            <label for="publish_date">publish_date:<span class="imp">*</span></label>
                            <input type="date" name="publish_date" id="publish_date" value="{{ $data->publish_date }}">
                        </div>
                        <div class="formGroup">
                            ...
                            <select class="custom-select" id="blog_status" name="blog_status">
                                ...
                                <option value="Public" {{($data->blog_status == 'Public') ? 'selected' : ''; }}>Public</option>
                                <option value="Private" {{($data->blog_status == 'Private') ? 'selected' : ''; }}>Private</option>
                            </select>
                        </div>
                        <div class="formGroup">
                            ...
                            <select class="custom-select" id="blog_type" name="blog_type">
                                ...
                                <option value="Business" {{($data->blog_type == 'Business') ? 'selected' : ''; }}>Business</option>
                                <option value="Couples" {{($data->blog_type == 'Couples') ? 'selected' : ''; }}>Couples</option>
                                <option value="Family" {{($data->blog_type == 'Family') ? 'selected' : ''; }}>Family</option>
                                <option value="Friends" {{($data->blog_type == 'Friends') ? 'selected' : ''; }}>Friends</option>
                                <option value="Solo" {{($data->blog_type == 'Solo') ? 'selected' : ''; }}>Solo</option>
                            </select>
                        </div>
                    </div>
                    <div class="formGroup">
                        <label for="blog_text">BLOG TEXT:<span class="imp">*</span></label>
                        <textarea type="text" cols="30" rows="10" name="blog_text" id="editor">{{ $data->blog_text }}</textarea>
                    </div>
                </div>
            </form>
            @if ($errors->any())
