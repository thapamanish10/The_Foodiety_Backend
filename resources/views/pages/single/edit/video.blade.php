...
@section('content')
    <main class="dashboardContainer">
        ...
        <div class="detailsformBody">
            <form action="{{ route('product.video.store', $data->id) }}" method="POST" enctype="multipart/form-data"
                id="" class="form">
                @csrf
                <div class="formHeading">
                    ...
                    <div class="buttonDiv">
                        <button type="button" class="btn btnCancle">
                            <span>Cancel</span>
                            <img src="{{ asset('dashboardicons/cancle.png') }}" alt="CancelIcon">
                        </button>
                        <button type="submit" class="btn btnUpdate btnPrimary">
                            <span>Update</span>
                            <img src="{{ asset('dashboardicons/update.png') }}" alt="ImageIcon">
                        </button>
                    </div>
                </div>
                <div class="formBody">
                    <div class="formGroup" hidden>
                        ...
                        <input type="text" value="{{ $product->id }}" name="product_id" id="product_id">
                    </div>
                    <div class="row">
                        <div class="formGroup">
                            <label for="video_name">VIDEO NAME: <span class="imp">*</span></label>
                            <input type="text" value="{{ $data->video_name }}" name="video_name" id="video_name">
                        </div>
                        <div class="formGroup">
                    ...
                    <div class="formGroupImage">
                        ...
                        @if ($data->video)
                            <video id="selectedVideo" src="{{ asset($data->video) }}" controls onclick="document.getElementById('video').click();" style="display: block; max-width: 100%; max-height: 300px; margin-top: 10px;"></video>
                        @else
